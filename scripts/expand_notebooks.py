...
# Notebooks config: (filename, title_short, difficulty, priority, mental_model, key_concept, next_step)
NOTEBOOKS = {
    '04_01_eda.ipynb': {
        ...
        'next_step': '**Next Step:** Proceed to `04_02_data_cleaning.ipynb` to learn how to fix the issues EDA reveals.'
    },
    '04_02_data_cleaning.ipynb': {
        ...
        'next_step': '**Next Step:** Proceed to `04_03_missing_values.ipynb` to learn how to handle missing data.'
    },
    '04_03_missing_values.ipynb': {
        ...
        'next_step': '**Next Step:** Proceed to `04_04_outliers.ipynb` to learn how to detect and handle outliers.'
    },
    '04_04_outliers.ipynb': {
        ...
        'next_step': '**Next Step:** Proceed to `04_05_feature_scaling_encoding.ipynb` to learn feature transformations.'
    },
    '04_05_feature_scaling_encoding.ipynb': {
        ...
        'next_step': '**Next Step:** Proceed to `04_06_train_val_test_splits.ipynb` to learn proper data splitting.'
    },
    '04_06_train_val_test_splits.ipynb': {
        ...
        'next_step': '**Next Step:** Proceed to `04_07_data_leakage.ipynb` to learn about avoiding data leakage.'
    },
    '04_07_data_leakage.ipynb': {
        ...
        'next_step': '**Next Step:** Proceed to `04_08_synthesis.ipynb` to apply everything you learned in a comprehensive exercise.'
    },
    '04_08_synthesis.ipynb': {
        ...
        'next_step': '**Next Step:** You have completed the Data Analysis & Preparation phase. Proceed to the next phase of your ML journey!'
    }
}


def make_new_md_cell(source_lines):
...
def process_title_cell(cell, config):
    """Modify the first cell (title cell) to add difficulty/priority and improve sections."""
    source = ''.join(cell['source'])

    # Update title with difficulty and priority
    old_title_pattern = r'# 04\.\d+ - .*'
    new_title = f"# {config['code']} - {config['title']}"
    source = re.sub(old_title_pattern, new_title, source)

    # Replace Learning Objectives with checkbox format
    lo_old = re.search(
        r'## 4\. Learning Objectives.*?(?=## 5\.)',
        source, re.DOTALL
    )
    if lo_old:
        lo_text = lo_old.group(0)
        # Extract the objectives as bullet points
...
"""
    ...
    # Replace Mental Model with proper analogy
    mm_old = re.search(
        r'## 5\. Mental Model.*?(?=## \d+\.|$)',
        source, re.DOTALL
    )
    if mm_old:
        mm_text = mm_old.group(0)
...
"""
    ...
    # Add Difficulty and Priority after Phase line
...
def process_last_cell(cell, config):
    """Modify the last cell to add Exit Criteria and Next Step."""
    source = ''.join(cell['source'])

    # Replace the "Closed-Book Recall" section with Knowledge Check
    kb_old = re.search(
        r'## 19\. Closed-Book Recall.*?(?=## 20\.)',
        source, re.DOTALL
    )
    if kb_old:
        kb_text = kb_old.group(0)
        kb_new = f"""## 19. Knowledge Check

{config['knowledge_check']}

"""
        source = source.replace(kb_text, kb_new)

    # Replace "Further Experiment" with Hands-On Practice
    he_old = re.search(
        r'## 22\. Further Experiment.*?(?=## 23\.)',
        source, re.DOTALL
    )
    if he_old:
        he_text = he_old.group(0)
        he_new = f"""## 22. Hands-On Practice

{config['hands_on']}

"""
        source = source.replace(he_text, he_new)

    # Add Exit Criteria and Next Step after Summary
    summary_old = re.search(
        r'## 21\. Summary.*?(?=## 22\.)',
        source, re.DOTALL
    )
    if summary_old:
        summary_text = summary_old.group(0)
        summary_new = f"""## 21. Summary
...
"""
        source = source.replace(summary_text, summary_new)

    cell['source'] = [source]
    return cell


def insert_decision_guidance(cells, config, after_section='Why Does This Matter'):
    """Insert Decision Guidance table and Common Mistakes after the 'Why Does This Matter' section."""
    for i, cell in enumerate(cells):
        if cell['cell_type'] == 'markdown':
            source = ''.join(cell['source'])
            if after_section in source:
                # Find the end of this cell's section (next ## or end of content)
                # Insert new cells after this one
                dg_cell = make_new_md_cell([
                    f"## 2a. Decision Guidance\n\n",
                    f"{config['decision_guidance']}\n"
                ])
                cm_cell = make_new_md_cell([
                    f"## 2b. Common Mistakes to Avoid\n\n",
...
def process_notebook(filename, config):
    ...
    # Step 1: Modify title cell (cell 0)
    cells[0] = process_title_cell(cells[0], config)

    # Step 2: Insert Decision Guidance and Common Mistakes after "Why Does This Matter"
    cells = insert_decision_guidance(cells, config)

    # Step 3: Modify last cell to add Exit Criteria and Next Step
    cells[-1] = process_last_cell(cells[-1], config)
