...
def process_notebook(filename, config):
    ...
    kb_match = re.search(
        r"## \d+\. Closed-Book Recall.*?(?=## \d+\. Teach-Back)", last_src, re.DOTALL
    )
    if kb_match:
    ...
    he_match = re.search(
        r"## \d+\. Further Experiment.*?(?=## \d+\. Verification)", last_src, re.DOTALL
    )
    if he_match:
    ...
    if summary_match:
        ...
        last_src = (
            last_src[: summary_match.start(1)] + summary_new + last_src[summary_match.start(2) :]
        )
